...
const initializeProviders = () => {
  try {
    if (sunnyConfig.isConfigured()) {
      geminiClient = new GoogleGenerativeAI(sunnyConfig.gemini.apiKey);
...
const callGemini = async (userMessage, metadata = {}) => {
  ...
  try {
    if (!geminiClient) {
        throw new Error('Gemini client not initialized. Check your GEMINI_API_KEY environment variable.');
    }
  ...
  return {
    providers: {
      gemini: {
        ...
        model: sunnyConfig.gemini.model,
        apiKeyConfigured: !!sunnyConfig.gemini.apiKey
      },
      openai: {
